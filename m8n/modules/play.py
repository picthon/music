...
async def hfmm(_, message):
    ...
    if status in ["ON", "on", "On"]:
        ...
        if message.chat.id not in DISABLED_GROUPS:
            await lel.edit(
                f" __- البوت غير شغال **{message.chat.title}**__"
            )
            return
...
async def play(_, message: Message):
    ...
    if message.sender_chat:
        return await message.reply_text(
            " __انت غير مشرف ** -ارجع مشرف- **!__\n│\n╰ وراها تكدر تشغل البوت."
        )

    if message.chat.id in DISABLED_GROUPS:
        await message.reply(
            " __**- المطور اوقف تشغيل البوت يرجى محادثه المطور لتشغيله! **__"
        )
        return
    ...
    try:
        ...
        if message.chat.username:
            try:
                ...
                await message.reply_text(
                    f" __**-  فشل انضمام حساب المساعد والسب *__\n\n**Reason**:{e}"
                )
                return
        else:
            try:
                ...
                return await message.reply_text(
                    f" __**- فشل انضمام حساب المساعد والسبب**__\n\n**Reason**:{e}"
                )
    ...
    if await is_active_chat(message.chat.id):
        ...
        await message.reply_photo(
            photo="https://telegra.ph/file/0f4e791a1ec3d7703674a.jpg",
    ...
    os.remove("https://telegra.ph/file/0f4e791a1ec3d7703674a.jpg")
