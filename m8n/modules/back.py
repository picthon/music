...
dbclean_keyboard = InlineKeyboardMarkup(
    [
        [
            ...
            InlineKeyboardButton("متأكد .", callback_data="cleandb"),],
        [    InlineKeyboardButton("غيࢪ متأكد .", callback_data="cbmenu"),
...
async def skipvc(_, CallbackQuery):
    ...
    if not a.can_manage_voice_chats:
        return await CallbackQuery.answer(
            """
‹ رجاءا لا تضغط اذا لم يكن لديك رتبة مشرف ›
""",
            show_alert=True,
        )
...
async def pausevc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.answer(f"- لا يـوجـد شـئ شغـال", show_alert=True)
            return
    else:
        await CallbackQuery.answer(f"- لا يـوجـد شـئ شغـال !", show_alert=True)
...
async def cbmenu(_, query: CallbackQuery):
    if query.message.sender_chat:
        return await query.answer("- لا يمكنك انت غير مشرف !\n\n- ارجع مشرف وشغل")
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer("‹ رجاءا لا تضغط اذا لم يكن لديك رتبة مشرف ›", show_alert=True)
    ...
    if is_music_playing(chat_id):
        ...
        await query.answer("- لا يـوجـد شـئ شغـال", show_alert=True)
...
async def dbconfirm(_, query: CallbackQuery):
    if query.message.sender_chat:
        return await query.answer("- انت غير مشرف !\n\n- ارجع مشرف وشغل .")
    a = await _.get_chat_member(query.message.chat.id, query.from_user.id)
    if not a.can_manage_voice_chats:
        return await query.answer("‹ رجاءا لا تضغط اذا لم يكن لديك رتبة مشرف ›", show_alert=True)
    chat_id = query.message.chat.id
