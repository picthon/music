...
@app.on_message(command(["تحديث", f"اعاده"]) & other_filters)
...
async def update_admin(client, message: Message):
    ...
    await message.reply_text(
        "✅ تـم اعادة **تشـغيل البـوت** !\n✅ وتـم **تحـديث** قـائمـة **المـشࢪفيـن.**"
    )
...
async def pause(_, message: Message):
    if message.sender_chat:
        return await message.reply_text(
            " __- انت غيـࢪ مشـࢪف!__\n│\n╰ اࢪجـع مشـࢪف عـادي وتـدلل"
        )
    ...
    await message.reply_text(
        f"• تـم ايقـاف التـشغـيل مـؤقـتا\n• By : {checking}"
    )
...
async def stop(_, message: Message):
    if message.sender_chat:
        return await message.reply_text(
            " __ - انت غيـࢪ مشـࢪف ماتـكدࢪ تستـخـدم البـوت !__\n│\n╰ ارجع مشرف وشغـل"
        )
    permission = "can_delete_messages"
    m = await adminsOnly(permission, message)
    if m == 1:
